...
SLATE_ZEBRA = RGBColor(0xF8, 0xFA, 0xFC)
AMBER_WARN = RGBColor(0xFE, 0xF3, 0xC7)
ICE_BLUE = RGBColor(0xF0, 0xF7, 0xFF)
GREEN_BANNER = RGBColor(0x00, 0x5C, 0x30)
GOLD = RGBColor(0xDA, 0xA5, 0x20)
CREAM = RGBColor(0xF4, 0xF9, 0xF4)

ACCENT_BG_HEX = 'C00000'
CHARCOAL_BG_HEX = '1E293B'
SLATE_BG_HEX = 'F8FAFC'
AMBER_BG_HEX = 'FEF3C7'
...
def apply_tuv_font(run, font_name='Arial', size_pt=None, bold=False, color=None, italic=False):
    """Set explicit Arial font on a run with optional size/bold/color/italic."""
...
def set_default_font(doc, font_name='Arial'):
    """Set Normal style font to Arial for the whole document."""
...
def add_tuv_section_bar(doc, text, level=1):
    """Create a TUV_RED filled bar with bold white Arial text."""
    p = doc.add_paragraph()
    p.paragraph_format.space_before = Pt(12)
    p.paragraph_format.space_after = Pt(6)
    pPr = p._p.get_or_add_pPr()
    shd = OxmlElement('w:shd')
    shd.set(qn('w:fill'), ACCENT_BG_HEX)
    shd.set(qn('w:val'), 'clear')
    pPr.append(shd)
    run = p.add_run(text)
    sizes = {1: 11, 2: 10, 3: 9}
    apply_tuv_font(run, size_pt=sizes.get(level, 11), bold=True, color=WHITE)
    return p


def add_tuv_divider(doc):
    """Add a dark charcoal horizontal divider line."""
    p = doc.add_paragraph()
...
def setup_document(doc, landscape=False, client_key=None):
    ...
    hp.text = ''
    apply_tuv_font(hp.add_run('TÜV AUSTRIA'), 'Arial', 8, bold=True, color=TUV_RED)
    apply_tuv_font(hp.add_run('  |  Audit Documentation'), 'Arial', 8, color=RGBColor(0x66, 0x66, 0x66))
...
def add_header_row(table, headers, client_key=None):
    ...
    for i, h in enumerate(headers):
        ...
        cell.text = ''
        p = cell.paragraphs[0]
        apply_tuv_font(p.add_run(h), size_pt=9, bold=True, color=WHITE)
        set_cell_shading(cell, ACCENT_BG_HEX)
...
def add_data_row(table, data, bold=False, color=None, client_key=None):
    ...
    for i, val in enumerate(data):
        ...
        cell.text = ''
        p = cell.paragraphs[0]
        apply_tuv_font(p.add_run(str(val)), size_pt=9, bold=bold, color=color or BLACK)
        if i % 2 == 0:
...
def add_section_heading(doc, text, client_key=None):
    return add_tuv_section_bar(doc, text, level=1 if len(text) < 30 else 2)


def add_sub_heading(doc, text, client_key=None):
    p = doc.add_paragraph()
    run = p.add_run(text)
    apply_tuv_font(run, size_pt=11, bold=True, color=TUV_RED)
    p.space_after = Pt(4)
    return p
...
def add_body_text(doc, text, bold=False, client_key=None):
    p = doc.add_paragraph()
    run = p.add_run(text)
    apply_tuv_font(run, size_pt=10, bold=bold, color=BLACK)
    p.space_after = Pt(4)
    return p
...
def add_bullet(doc, text, client_key=None):
    ...
    p.clear()
    run = p.add_run(text)
    apply_tuv_font(run, size_pt=10, color=BLACK)
    return p
...
# ── Surveillance Audit generators ─────────────────────────────────────────

def generate_surveillance_plan(data, output_path, client_key: str = None):
    client = get_client(client_key) if client_key else None
    lang = client.language if client else "en"

    doc = Document()
    setup_document(doc, client_key=client_key)
    cycle = data.get('surveillance_cycle', 'Surveillance Audit')
    add_cover_page(doc, f'Surveillance Audit Plan - {cycle}',
                   data.get('client_name', 'N/A'),
                   data.get('standard', 'N/A'),
                   data.get('audit_date', 'N/A'),
                   client_key=client_key)

    add_section_heading(doc, "Audit Objectives", client_key=client_key)
    for obj in data.get('audit_objectives', []):
        add_bullet(doc, obj, client_key=client_key)

    add_section_heading(doc, "Audit Scope (Reduced)", client_key=client_key)
    add_body_text(doc, data.get('audit_scope', 'N/A'), client_key=client_key)

    add_section_heading(doc, "Audit Criteria", client_key=client_key)
    for c in data.get('audit_criteria', []):
        add_bullet(doc, c, client_key=client_key)

    review_ncs = data.get('review_of_open_ncs', '')
    if review_ncs:
        add_section_heading(doc, "Review of Open Nonconformities", client_key=client_key)
        add_body_text(doc, review_ncs, client_key=client_key)

    changes = data.get('review_of_changes', '')
    if changes:
        add_section_heading(doc, "Review of Changes", client_key=client_key)
        add_body_text(doc, changes, client_key=client_key)

    add_section_heading(doc, "Audit Team", client_key=client_key)
    table = doc.add_table(rows=1, cols=3)
    table.style = 'Table Grid'
    table.alignment = WD_TABLE_ALIGNMENT.CENTER
    add_header_row(table, TABLE_HEADERS[lang]["audit_team"], client_key=client_key)
    ...
    add_section_heading(doc, "Daily Schedule", client_key=client_key)
    table = doc.add_table(rows=1, cols=7)
    table.style = 'Table Grid'
    table.alignment = WD_TABLE_ALIGNMENT.CENTER
    add_header_row(table, TABLE_HEADERS[lang]["daily_schedule"], client_key=client_key)
    ...
    add_section_heading(doc, "Confidentiality", client_key=client_key)
    add_body_text(doc, data.get('confidentiality', CONFIDENTIALITY[lang]), client_key=client_key)

    add_section_heading(doc, "Language & Report", client_key=client_key)
    add_body_text(doc, f"Language: {data.get('language', 'English')}", client_key=client_key)
    add_body_text(doc, f"Report Due: {data.get('report_date', 'N/A')}", client_key=client_key)
...
def generate_surveillance_report(data, output_path, client_key: str = None):
    client = get_client(client_key) if client_key else None
    lang = client.language if client else "en"
    standard = data.get('standard', 'the applicable standard')
    ...
    doc = Document()
    setup_document(doc, client_key=client_key)
    add_cover_page(doc, 'Surveillance Audit Report', data.get('client_name', 'N/A'),
                   data.get('standard', 'N/A'), data.get('audit_date', 'N/A'),
                   data.get('lead_auditor', ''),
                   client_key=client_key)
    ...
    add_section_heading(doc, "Executive Summary", client_key=client_key)
    add_body_text(doc, data.get('findings_summary', ''), client_key=client_key)

    prev_ncs = data.get('previously_raised_ncs', '')
    if prev_ncs:
        add_section_heading(doc, "Previously Raised Nonconformities", client_key=client_key)
        add_body_text(doc, prev_ncs, client_key=client_key)

    changes = data.get('changes_since_initial', '')
    if changes:
        add_section_heading(doc, "Changes Since Initial Certification", client_key=client_key)
        add_body_text(doc, changes, client_key=client_key)

    add_section_heading(doc, "Audit Methodology", client_key=client_key)
    default_methodology = {
        'approach': METHODOLOGY[lang]["approach"].format(standard=standard),
        'sampling': METHODOLOGY[lang]["sampling"].format(standard=standard),
        'criteria': METHODOLOGY[lang]["criteria"].format(standard=standard),
    ...
    methodology = data.get('methodology', default_methodology)
    for key, label in [('approach', 'approach'), ('sampling', 'sampling'), ('criteria', 'criteria'), ('methods', 'methods')]:
        add_sub_heading(doc, t(label, lang), client_key=client_key)
        add_body_text(doc, methodology.get(key, ''), client_key=client_key)

    if data.get('positive_findings'):
        add_section_heading(doc, "Positive Findings", client_key=client_key)
        for pf in data['positive_findings']:
            add_bullet(doc, pf, client_key=client_key)

    if data.get('opportunities_for_improvement'):
        add_section_heading(doc, "Opportunities for Improvement", client_key=client_key)
        for ofi in data['opportunities_for_improvement']:
            add_bullet(doc, ofi, client_key=client_key)

    if data.get('nonconformities'):
        add_section_heading(doc, "Nonconformities", client_key=client_key)
        table = doc.add_table(rows=1, cols=4)
        table.style = 'Table Grid'
        add_header_row(table, TABLE_HEADERS[lang]["nc_table"], client_key=client_key)
        for nc in data['nonconformities']:
    ...
    cb_rec = data.get('cb_recommendation', 'Continued Certification')
    add_section_heading(doc, "CB Recommendation", client_key=client_key)
    add_body_text(doc, cb_rec, client_key=client_key)

    add_section_heading(doc, "Conclusion", client_key=client_key)
    add_body_text(doc, data.get('conclusion', ''), client_key=client_key)
...
def generate_surveillance_findings_summary(data, output_path, client_key: str = None):
    client, lang, rtl, header_color = _resolve_client(client_key)

    doc = Document()
    setup_document(doc, client_key=client_key)
    add_cover_page(doc, 'Surveillance Findings Summary',
                   data.get('client_name', 'N/A'),
                   data.get('standard', 'N/A'),
                   data.get('audit_date', 'N/A'),
                   client_key=client_key)
    ...
    if cycle:
        add_section_heading(doc, f"Cycle: {cycle}", client_key=client_key)

    add_section_heading(doc, "Findings Summary", client_key=client_key)
    summary = data.get('summary', {})
    if summary:
        add_body_text(doc, f"Total Findings: {summary.get('total_findings', 0)}", client_key=client_key)
        add_body_text(doc, f"Major: {summary.get('major', 0)}", client_key=client_key)
        add_body_text(doc, f"Minor: {summary.get('minor', 0)}", client_key=client_key)
        add_body_text(doc, f"OFI: {summary.get('ofi', 0)}", client_key=client_key)
        add_body_text(doc, f"Observations: {summary.get('observations', 0)}", client_key=client_key)
        add_body_text(doc, f"Recurring NCs: {summary.get('recurring_ncs', 0)}", client_key=client_key)

    findings = data.get('findings', [])
    if findings:
        add_section_heading(doc, "Findings Register", client_key=client_key)
        table = doc.add_table(rows=1, cols=7)
        table.style = 'Table Grid'
        add_header_row(table, ['Finding ID', 'Clause', 'Type', 'Severity', 'Description', 'Status', 'Previous NC'], client_key=client_key)
        for f in findings:
            ftype = f.get('type', '')
            row_color = TUV_RED if ftype == 'NC' else (RGBColor(0xCC, 0x7A, 0x00) if ftype == 'OFI' else None)
            add_data_row(table, [
                f.get('finding_id', ''),
                f.get('clause', ''),
                ftype,
...
# ── Template-driven fill for fixed TÜV forms ──────────────────────────────

def _fill_fixed_template(doc_type, data, template_path, output_dir, safe_name):
    """Fill a fixed TÜV form (plan/report/participation/certificate) preserving layout.

    NOTE: clean_docx is skipped — TÜV templates are production-clean Office exports
    and their glossed webSettings.xml / customXml references must be preserved intact.
    """
    from app.services.template_filler import fill_template
    filename = f'{doc_type}_{safe_name}.docx'
    output_path = os.path.join(output_dir, filename)
    if fill_template(doc_type, data, template_path, output_path) and os.path.exists(output_path):
        return output_path
    return None
...
GENERATORS = {
    ...
    'Surveillance_Plan': generate_surveillance_plan,
    'Surveillance_Report': generate_surveillance_report,
    'Surveillance_Findings_Summary': generate_surveillance_findings_summary,
}


def clean_docx(output_path):
    """Remove legacy python-docx artifacts for cross-platform compatibility with Word and Google Docs."""
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            rels_path = os.path.join(tmp, '_rels', '.rels')
            doc_rels = os.path.join(tmp, 'word', '_rels', 'document.xml.rels')
            types_path = os.path.join(tmp, '[Content_Types].xml')

            remove_targets = ['stylesWithEffects', 'webSettings.xml', 'themeOverride.xml', 'customXml', 'thumbnail.jpeg']

            # Remove customXml directory
            custom_xml_dir = os.path.join(tmp, 'customXml')
            ...
            for root, dirs, files in os.walk(tmp):
                for f in files:
                    ...
                    if any(t in f or t in rel for t in remove_targets):
                        os.remove(fp)

            # Clean relationships files
            for xml_path in [rels_path, doc_rels]:
                if not os.path.exists(xml_path):
                    continue
                tree = ET.parse(xml_path)
                root_el = tree.getroot()
                to_remove = []
                for rel in root_el.findall(f'{NS_REL}Relationship'):
                    target = rel.get('Target', '')
                    if any(t in target for t in remove_targets):
            ...
            if os.path.exists(types_path):
                ...
                root_el = tree.getroot()
                to_remove = []
                for ov in root_el.findall(f'{{{TYPES_NS}}}Override'):
                    pn = ov.get('PartName', '')
                    if any(t in pn for t in remove_targets):
                        to_remove.append(ov)
                for ov in to_remove:
                    root_el.remove(ov)
        ...
        logger.warning(f'clean_docx failed for {output_path}: {e}')
        return False
...
def generate_document_file(doc_type, data, output_dir, template_path=None, standard_key=None, client_key: str = None):
    ...
    # Fixed TÜV forms take precedence (layout must not change)
    if effective_template and effective_template.endswith('.docx') and doc_type not in ('ISO_Checklist',):
        filled = _fill_fixed_template(doc_type, data, effective_template, output_dir, safe_name)
        if filled:
            return filled
