...
export default function Surveillance({ API }) {
      ...
      <div className="animate-fadeIn">
        ...
        <div className="rounded-xl p-6 mb-5 bg-[var(--bg-card)] border border-[var(--border-color)]">
          <div className="flex justify-between items-start">
            <div>
              <h2 className="text-base font-semibold mb-4 text-[var(--text-primary)] m-0">Cycle {selectedCycle.cycle_number}: {CYCLE_LABELS[selectedCycle.cycle_number] || 'Surveillance'}</h2>
              <div className="text-sm text-[var(--text-secondary)]">
                {project?.title || selectedCycle.project_id} · Scheduled: {selectedCycle.scheduled_date}
              </div>
            </div>
            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium" style={{ background: STATUS_COLORS[selectedCycle.status] + '20', color: STATUS_COLORS[selectedCycle.status], fontSize: 13, padding: '5px 12px' }}>
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          <button className="bg-gray-100 text-gray-700 border border-gray-300 hover:bg-gray-200 dark:bg-gray-200 dark:text-gray-800 px-5 py-2.5 rounded-lg text-sm font-semibold inline-flex items-center justify-center transition-all duration-150 cursor-pointer whitespace-nowrap" onClick={() => loadScope(selectedCycle.id)}>🔍 Generate Scope</button>
          <button className="bg-gray-100 text-gray-700 border border-gray-300 hover:bg-gray-200 dark:bg-gray-200 dark:text-gray-800 px-5 py-2.5 rounded-lg text-sm font-semibold inline-flex items-center justify-center transition-all duration-150 cursor-pointer whitespace-nowrap" onClick={() => setShowAddFinding(!showAddFinding)}>+ Add Finding</button>
          {selectedCycle.status === 'scheduled' && (
            <button className="bg-[var(--primary)] text-white hover:bg-[var(--primary-dark)] disabled:opacity-50 disabled:cursor-not-allowed px-5 py-2.5 rounded-lg text-sm font-semibold inline-flex items-center justify-center border-none transition-all duration-150 cursor-pointer whitespace-nowrap" onClick={() => handleUpdateStatus(selectedCycle.id, 'in_progress')}>▶ Start Audit</button>
          )}
          {selectedCycle.status === 'in_progress' && (
            <button className="bg-[var(--primary)] text-white hover:bg-[var(--primary-dark)] disabled:opacity-50 disabled:cursor-not-allowed px-5 py-2.5 rounded-lg text-sm font-semibold inline-flex items-center justify-center border-none transition-all duration-150 cursor-pointer whitespace-nowrap" onClick={() => handleUpdateStatus(selectedCycle.id, 'completed')}>✓ Complete</button>
          )}
        </div>

        {scope && (
          <div className="rounded-xl p-6 mb-5 bg-[var(--bg-card)] border border-[var(--border-color)]">
            <h3 className="text-base font-semibold mb-4 text-[var(--text-primary)] m-0">Reduced Scope</h3>
            ...
            {scope.manday_reduction_pct > 0 && (
              <div className="mt-2 text-xs text-[var(--text-secondary)]">Manday reduction: {scope.manday_reduction_pct}%</div>
            )}
          </div>
        )}
        ...
        {showAddFinding && (
          <div className="rounded-xl p-6 mb-5 bg-[var(--bg-card)] border border-[var(--border-color)]">
            <h3 className="text-base font-semibold mb-4 text-[var(--text-primary)] m-0">Add Finding</h3>
            ...
            <div className="mb-5 mt-2">
              ...
              <textarea value={findingForm.description} rows={3} placeholder="Detailed finding description..."
                ...
                className="w-full px-3 py-2 rounded-md border border-[var(--border-color)]" style={{ resize: 'vertical' }} />
            </div>
            <div className="flex gap-2 mt-2">
              <button className="bg-[var(--primary)] text-white hover:bg-[var(--primary-dark)] disabled:opacity-50 disabled:cursor-not-allowed px-5 py-2.5 rounded-lg text-sm font-semibold inline-flex items-center justify-center border-none transition-all duration-150 cursor-pointer whitespace-nowrap" onClick={handleAddFinding}>Add Finding</button>
              <button className="bg-gray-100 text-gray-700 border border-gray-300 hover:bg-gray-200 dark:bg-gray-200 dark:text-gray-800 px-5 py-2.5 rounded-lg text-sm font-semibold inline-flex items-center justify-center transition-all duration-150 cursor-pointer whitespace-nowrap" onClick={() => setShowAddFinding(false)}>Cancel</button>
            </div>
          </div>
        )}

        <div className="rounded-xl p-6 mb-5 bg-[var(--bg-card)] border border-[var(--border-color)]">
          <h3 className="text-base font-semibold mb-4 text-[var(--text-primary)] m-0">Findings ({findings.length})</h3>
          {findings.length === 0 ? (
            <div className="text-center p-12 text-[var(--text-secondary)]">No findings recorded yet</div>
          ) : (
                ...
                <div key={f.id} className="p-2.5 rounded-md" style={{ border: '1px solid var(--border-color)', borderLeft: `4px solid ${FINDING_TYPE_COLORS[f.finding_type] || '#999'}` }}>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-1.5">
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium" style={{ background: FINDING_TYPE_COLORS[f.finding_type] + '20', color: FINDING_TYPE_COLORS[f.finding_type], fontSize: 10, padding: '2px 8px' }}>
                        {f.finding_type.replace('_', ' ')}</span>
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium" style={{ background: f.severity === 'Major' ? TUV_RED + '20' : '#F39C12' + '20', color: f.severity === 'Major' ? TUV_RED : '#F39C12', fontSize: 10, padding: '2px 8px' }}>
                        {f.severity}</span>
                    </div>
                  </div>
  ...
  return (
    <div className="animate-fadeIn">
      <div className="mb-8">
        <h2 className="text-3xl font-bold text-[var(--text-primary)] m-0">🔄 Surveillance & Recertification</h2>
        <p className="mt-1 text-[var(--text-secondary)]">Manage post-certification surveillance audit cycles and recertification</p>
      </div>

      {stats && (
        <div className="grid gap-5 mb-8 grid-cols-[repeat(auto-fit,minmax(220px,1fr))]">
          <div className="rounded-xl p-6 bg-[var(--bg-card)] border border-[var(--border-color)]"><div className="text-3xl font-bold text-[var(--primary)]">{stats.total_cycles ?? 0}</div><h3 className="text-xs font-semibold uppercase tracking-wider mb-2 text-[var(--text-secondary)]">Total Cycles</h3></div>
          <div className="rounded-xl p-6 bg-[var(--bg-card)] border border-[var(--border-color)]"><div className="text-3xl font-bold text-[var(--primary)]" style={{ color: '#3498DB' }}>{stats.upcoming_30d ?? 0}</div><h3 className="text-xs font-semibold uppercase tracking-wider mb-2 text-[var(--text-secondary)]">Upcoming (30d)</h3></div>
          <div className="rounded-xl p-6 bg-[var(--bg-card)] border border-[var(--border-color)]"><div className="text-3xl font-bold text-[var(--primary)]" style={{ color: TUV_RED }}>{stats.overdue ?? 0}</div><h3 className="text-xs font-semibold uppercase tracking-wider mb-2 text-[var(--text-secondary)]">Overdue</h3></div>
          <div className="rounded-xl p-6 bg-[var(--bg-card)] border border-[var(--border-color)]"><div className="text-3xl font-bold text-[var(--primary)]" style={{ color: '#27AE60' }}>{stats.completed ?? 0}</div><h3 className="text-xs font-semibold uppercase tracking-wider mb-2 text-[var(--text-secondary)]">Completed</h3></div>
        </div>
      )}
      ...
      <div className="flex gap-2 mb-4 flex-wrap items-center">
        ...
        <button className="bg-gray-100 text-gray-700 border border-gray-300 hover:bg-gray-200 dark:bg-gray-200 dark:text-gray-800 px-5 py-2.5 rounded-lg text-sm font-semibold inline-flex items-center justify-center transition-all duration-150 cursor-pointer whitespace-nowrap" onClick={async () => { await fetch(`${API}/surveillance/check-overdue`, { method: 'POST' }); loadData() }}>
          ⚠ Check Overdue
        </button>
      </div>

      <div className="rounded-xl p-6 mb-5 bg-[var(--bg-card)] border border-[var(--border-color)]">
        <h3 className="text-base font-semibold mb-4 text-[var(--text-primary)] m-0">Auto-Schedule Surveillance</h3>
        <p className="text-xs text-[var(--text-secondary)]">Create 3 cycles (Y1, Y2 surveillance + Y3 recert) from certification date</p>
        <div className="flex gap-2 mt-2 flex-wrap">
          ...
          <button className="bg-[var(--primary)] text-white hover:bg-[var(--primary-dark)] disabled:opacity-50 disabled:cursor-not-allowed px-5 py-2.5 rounded-lg text-sm font-semibold inline-flex items-center justify-center border-none transition-all duration-150 cursor-pointer whitespace-nowrap" onClick={handleAutoSchedule} disabled={!autoScheduleProj || !autoScheduleDate}>Auto-Schedule</button>
        </div>
      </div>

      {showCreate && (
        <div className="rounded-xl p-6 mb-5 bg-[var(--bg-card)] border border-[var(--border-color)]">
          <h3 className="text-base font-semibold mb-4 text-[var(--text-primary)] m-0">Create Surveillance Cycle</h3>
          <div className="grid grid-cols-2 gap-3">
            <div className="mb-5">
              <label>Project *</label>
          ...
          <button className="bg-[var(--primary)] text-white hover:bg-[var(--primary-dark)] disabled:opacity-50 disabled:cursor-not-allowed px-5 py-2.5 rounded-lg text-sm font-semibold inline-flex items-center justify-center border-none transition-all duration-150 cursor-pointer whitespace-nowrap mt-3" onClick={handleCreate}>Create Cycle</button>
        </div>
      )}

      {cycles.length === 0 ? (
        <div className="rounded-xl p-6 mb-5 bg-[var(--bg-card)] border border-[var(--border-color)] text-center">
          <div className="text-5xl mb-3">🔄</div>
          <h3 className="text-base font-semibold mb-4 text-[var(--text-primary)] m-0">No surveillance cycles yet</h3>
          <p className="text-[var(--text-secondary)]">Create a cycle manually or auto-schedule from a certified project</p>
        </div>
      ) : (
        <div className="flex flex-col gap-2">
          {cycles.map(c => {
            ...
            return (
              <div key={c.id} className="rounded-xl p-6 mb-5 bg-[var(--bg-card)] border border-[var(--border-color)] flex items-center justify-between cursor-pointer"
                ...
                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium" style={{ background: STATUS_COLORS[c.status] + '20', color: STATUS_COLORS[c.status], fontSize: 11, padding: '4px 10px' }}>
                  {c.status}
                </span>
              </div>
            )
